...
一段「建筑生长」动画：砖、石、木、瓦从散落状态飞向各自的结构位置，由下至上完成装配，
最终长成一座中世纪城堡；镜头是一台缓慢环绕的转台，从近乎正立面的角度起转，
随着建筑长高慢慢停稳在四分之三视角。外面套一层编辑排版（标题 / 阶段标签 /
进度条 / 方位角读数 / 比例小人）。
...
**相机**（`CAM`）是绕 Y 轴的正交转台。正交投影下所有立方体的顶点屏幕偏移完全相同，
所以每帧只算一次八个顶点，其余方块直接平移复用；朝向相机的两个侧面按方位角象限选择，
落在屏幕右侧的那面拿亮色。画家算法排序只取决于相机所在象限，四种顺序在建模时各排一次，
逐帧零排序开销。

想改成固定镜头，把 `CAM.from` 和 `CAM.to` 设成同一个角度即可（`-45` 就是经典四分之三
视角）；想要不停歇的整圈转台，用 `from:-405, to:-45, bias:1`。注意 0° 与 ±90° 是正立面
死角，扫程尽量避开建筑最出彩的时段。
