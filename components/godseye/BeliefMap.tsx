  ...
  return (
    <div className="relative w-full h-full min-h-[280px] rounded-2xl border border-slate-800 bg-slate-950/70 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-40 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at 30% 20%, rgba(76,29,149,0.22), transparent 60%), radial-gradient(ellipse at 75% 80%, rgba(251,191,36,0.06), transparent 55%)' }}
      />
          ...
          >
            ...
            <span
              className={`absolute inset-0 -m-3 rounded-full blur-md transition-all duration-700 ${
                lit ? 'bg-amber-400/40 animate-pulse-glow' : 'bg-violet-900/50 animate-pulse'
              } ${selected ? 'scale-150' : ''}`}
            />
            {/* 星体 */}
            <span
              className={`relative block w-3.5 h-3.5 rounded-full border transition-all duration-700 ${
                lit
                  ? 'bg-amber-300 border-amber-200 shadow-[0_0_10px_rgba(251,191,36,0.7)]'
                  : 'bg-slate-800 border-violet-800 shadow-[0_0_6px_rgba(76,29,149,0.7)]'
              } ${selected ? 'ring-2 ring-amber-300 ring-offset-2 ring-offset-slate-950' : ''} group-hover:scale-125`}
            />
            {/* 悬停标签 */}
      ...
      <div className="absolute bottom-2 left-3 flex gap-4 text-[10px] text-slate-500 pointer-events-none">
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-slate-800 border border-violet-800" /> 暗星 · 待清除
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-amber-300 shadow-[0_0_5px_rgba(251,191,36,0.8)]" /> 已点亮
        </span>
      </div>
    </div>
  );
